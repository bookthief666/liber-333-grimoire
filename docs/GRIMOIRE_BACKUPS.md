...
When all three historical rows exist, migration orders them by timestamp for role inference, assigns one stable consultation ID and the Thesis, Antithesis, and Synthesis positions, then leaves the journal in its original newest-first presentation order. Each row keeps its original timestamp under `legacyTriadRecovered: true`; migration does not rewrite historical timestamps. Older closure-based saves can contain only one surviving row, and interrupted or separately produced backups can contain two. Those incomplete groups are retained as `legacyTriadFragment: true` with an internal, full-identity-derived `legacyTriadFragmentId` boundary identity: their recoverable reading content remains portable, they remain JSON/Markdown exportable, and no missing chapter or spread position is invented. Rows sharing a boundary ID must also share the legacy signature and remain within the time tolerance. Shared grouping, counts, deletion, retention, storage, and backup round trips honor that boundary identity. Older marked records that predate boundary IDs receive conservative per-row identities instead of being silently regrouped. The Workbench identifies them as “Historical Triad fragment.” If a later import supplies additional recoverable rows—with or without overlapping row IDs—unequal persisted boundaries remain separate unless their complete global membership proves one wholly local one-row/two-row fragment and one wholly imported complementary two-row/one-row fragment form exactly one unambiguous, adjacent, strictly time-ordered, recoverable three-row group; explicit cumulative overlap recovery remains supported, including already proven equal-time roles. Matching local rows receive recovered structure while all local fields, including favorite, note, and future-compatible reading data, win. If any complete or still-id-less mixed local/imported consultation cannot fit under the cap, its original local rows are retained and the oldest wholly imported consultation is omitted first when space is needed; an import fails safely only when no imported consultation can be displaced without touching local data. Storage writes and reloads preserve already marked post-retention fragment boundaries; genuinely raw, unmarked legacy rows still receive normal migration.
